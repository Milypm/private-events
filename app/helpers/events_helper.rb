...
module EventsHelper
  ...
  def attend_btn
    attend_btn = button_to 'Attend Event', attend_event_path(@event.id),
                           params: { attended_event_id: @event.id, event_attendee_id: current_user.id }, class: 'btn btn-success'
  ...
  def unattend_btn
    if EventAttending.exists?(attended_event_id: @event.id, event_attendee_id: current_user.id)
      button_to 'Unattend Event', cancel_event_attending_path(@event.id),
                params: { attended_event_id: @event.id, event_attendee_id: current_user.id }, method: :delete, class: 'btn btn-outline-secondary btn-sm mt-2'
    end
  end
end
